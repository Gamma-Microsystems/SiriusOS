...
with tarfile.open('ramdisk.igz','w:gz') as ramdisk:
    ...
    ramdisk.add('userland',arcname='/src/userland',filter=file_filter)
    ramdisk.add('kernel',arcname='/src/kernel',filter=file_filter)
    ramdisk.add('ld',arcname='/src/ld',filter=file_filter)
    ...
    ramdisk.addfile(symlink('bin/sh','nesh'))
